...
func build(pkg, dest, version, buildOS, buildArch string) {
	...
	timeFlag := fmt.Sprintf("-X github.com/ggncnt/go/support/app.buildTime=%s", buildTime)
	versionFlag := fmt.Sprintf("-X github.com/ggncnt/go/support/app.version=%s", version)
